...
function isIPv4Private(address: string): boolean {
  ...
  return (
    ...
    (parts[0] === 172 && parts[1] >= 16 && parts[1] <= 31) || // Used for local communications within a private network (RFC 1918) - 172.16.0.0/12
...
export function makeSecureDispatcher(
  ...
  const agentOpts: undici.Agent.Options = {
    // maxRedirections is handled at the fetch level, not the Agent level
  };
